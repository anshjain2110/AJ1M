    ...
    <section className="store w-full" style={{ fontFamily: "'Outfit', Inter, sans-serif", background: 'var(--lj-bg)' }} data-testid="home-engagement-section">
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-14 md:py-20">
        <div className="flex items-end justify-between mb-8">
          ...
          <Link to="/collections/engagement-rings" className="hidden sm:flex items-center gap-1.5 text-[13px] font-medium no-underline" style={{ color: 'var(--lj-accent)' }} data-testid="home-shop-all-engagement">
        ...
        {subCollections.length > 0 && (
          <div className="grid grid-cols-4 gap-3 md:gap-6 mb-10">
            {subCollections.slice(0, 5).map((s) => (
              <Link key={s.slug} to={`/collections/${s.slug}`} className="group text-center no-underline" style={{ color: 'inherit' }} data-testid={`home-subcol-${s.slug}`}>
        ...
        <div className="mt-9 text-center sm:hidden">
          <Link to="/collections/engagement-rings" className="inline-flex items-center gap-1.5 px-7 py-3.5 text-[13px] tracking-wide font-medium no-underline" style={{ background: 'var(--lj-accent)', color: '#fff' }}>
