...
/* Main page                                                           */
/* ================================================================== */
export default function ProjectDetailPageV2({ initialProject = null, initialSale = null, initialInfo = null, initialSlug = null }) {
  ...
  // Seed the initial variant from the server-provided project so the SSR HTML
  // renders a real "From $X" price that matches the Product JSON-LD (no $0 flash,
  // no visible/structured-data mismatch, no hydration mismatch on the price node).
  const seedTiers = availableTiers(initialProject?.price_matrix || {});
  const seedTier = seedTiers[0];
  const [tier, setTier] = useState(seedTier?.id || '');
  const [color, setColor] = useState(seedTier?.colors?.length ? 'Yellow' : '');
  const [carat, setCarat] = useState(() => {
    if (!seedTier) return '';
    if (initialProject && !typeHasCarat(initialProject.product_type)) return METAL_ONLY_KEY;
    const cs = availableCaratsForTier(initialProject?.price_matrix || {}, seedTier.id);
    return cs[0] || '';
  });
  ...
  // Load project + sale in parallel (skipped when server provided initialProject)
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: '#FBF7F0', fontFamily: "'Outfit', Inter, system-ui, sans-serif" }} data-testid="project-detail-v2">
      ...
      {/* Product/Breadcrumb JSON-LD is emitted once, server-side, by the Next
          route (app/projects/[slug]/page.jsx) so it appears in the initial HTML.
          AggregateRating/Review schema is intentionally NEVER emitted: the
          on-page reviews here are not a first-party review system. */}
