...
export default function LoginClient() {
  ...
  useEffect(() => {
    axios.get('/api/auth/google/config')
  ...
  const handleSendOtp = async () => {
    ...
    setError(''); setLoading(true);
    try {
      const res = await axios.post('/api/auth/request-otp', { identifier: identifier.trim() });
  ...
  const verify = async (code) => {
    setError(''); setLoading(true);
    try {
      const res = await axios.post('/api/auth/verify-otp', { identifier: identifier.trim(), otp_code: code });
      finishLogin(res);
    } catch (err) {
    ...
    <GoogleOAuthProvider clientId={googleCfg.client_id}>
      <div className="w-full flex justify-center" data-testid="google-login-button">
        <GoogleLogin onSuccess={handleGoogleSuccess} onError={() => setError('Google sign-in was cancelled')}
          theme="outline" size="large" shape="pill" width="356" text="continue_with" />
      </div>
    </GoogleOAuthProvider>
  ) : null;

  return (
    <div className="min-h-screen flex flex-col" style={{ background: '#FBF7F0', fontFamily: "'Outfit', Inter, system-ui, sans-serif" }} data-testid="login-page">
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-10">
        <div className="w-full max-w-[420px]">
          ...
          <div className="text-center mt-6 space-y-3">
            <Link href="/" className="text-[13px] block w-full" style={{ color: '#6B746F' }}>← Back to home</Link>
            <a href="/admin/login" className="text-[13px] block w-full" style={{ color: '#6B746F', opacity: 0.6 }}>Admin Login</a>
          </div>
        </div>
      </div>
    </div>
