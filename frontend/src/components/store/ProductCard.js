...
export default function ProductCard({ product }) {
  ...
  const quickAdd = (e) => {
    ...
    addItem({
      ...
      price: product.price,
      image: img,
      metal: (product.metals && product.metals[0]) || '',
      carat: (product.carats && product.carats[0]) || '',
  ...
  return (
    <div className="group cursor-pointer" onClick={() => navigate(`/products/${product.slug}`)} data-testid={`product-card-${product.slug}`}>
