...
export default function LandingScreen() {
  ...
  const handleStartWizard = async () => { trackEvent('tlj_cta_start_click', { cta_id: 'hero_cta' }); await startWizard(); };
  React.useEffect(() => { trackEvent('tlj_landing_view', { session_id: state.sessionId }); }, [state.sessionId]);

  return (
    <div className="flex-1 flex flex-col">
      <div className="relative px-4 pt-8 pb-12 flex flex-col items-center text-center">
        <div className="w-48 h-48 md:w-56 md:h-56 rounded-full overflow-hidden mb-8 mx-auto" style={{ boxShadow: '0 0 60px rgba(15,94,76,0.1), var(--lj-shadow-2)', border: '2px solid rgba(15,94,76,0.15)' }}>
          <img src={HERO_IMAGE} alt="Premium diamond ring" className="w-full h-full object-cover" loading="eager" />
        </div>
        <h1 className="text-[28px] leading-[34px] font-semibold tracking-[-0.01em] mb-4 max-w-md" style={{ color: 'var(--lj-text)' }}>Get Your Dream Ring Without the Markup</h1>
        <p className="text-[16px] leading-[24px] mb-8 max-w-sm" style={{ color: 'var(--lj-muted)' }}>Same GIA & IGI Certified diamonds. Same quality settings. Fraction of the price.</p>
        ...
        <button onClick={handleStartWizard} data-testid="landing-start-wizard-button" className="w-full max-w-sm min-h-[52px] px-6 rounded-[14px] font-medium text-[16px] flex items-center justify-center gap-2 transition-all duration-300 active:scale-[0.99]" style={{ background: 'var(--lj-accent)', color: '#FFFFFF', boxShadow: '0 4px 20px rgba(15,94,76,0.2)' }}>
          Get My Free Quote <ArrowRight size={18} />
        </button>
        <a href="https://thelocaljewel.com" target="_blank" rel="noopener noreferrer" className="mt-4 text-[13px] leading-[18px] flex items-center gap-1 transition-colors duration-300" style={{ color: 'var(--lj-muted)' }}>or browse our collection <ChevronRight size={14} /></a>
      </div>
      ...
      <div className="px-4 py-10">
        ...
        <div className="space-y-4 max-w-md mx-auto">
          {[{ name: 'Sarah M.', text: 'AJ found me the perfect engagement ring at nearly half the price of what I was quoted at a retail jeweler. The quality is incredible.', rating: 5 }, { name: 'Mike T.', text: "Saved over $6,000 on my wife's anniversary ring. Same GIA certified diamond, better price. Highly recommend.", rating: 5 }, { name: 'Jessica L.', text: 'The personalized service was outstanding. AJ took the time to understand exactly what I wanted and delivered beyond expectations.', rating: 5 }].map((review, i) => (
            <div key={i} className="p-5 rounded-[14px]" style={{ background: 'var(--lj-surface)', border: '1px solid var(--lj-border)' }}>
              <div className="flex items-center gap-1 mb-3">{Array.from({ length: review.rating }).map((_, j) => <Star key={j} size={14} fill="var(--lj-accent)" style={{ color: 'var(--lj-accent)' }} />)}</div>
      ...
      <div className="px-4 pb-10 flex justify-center">
        <button onClick={handleStartWizard} data-testid="landing-bottom-cta" className="w-full max-w-sm min-h-[52px] px-6 rounded-[14px] font-medium text-[16px] flex items-center justify-center gap-2 transition-all duration-300 active:scale-[0.99]" style={{ background: 'var(--lj-accent)', color: '#FFFFFF', boxShadow: '0 4px 20px rgba(15,94,76,0.2)' }}>
