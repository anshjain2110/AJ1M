    ...
    <div className="flex-1 flex flex-col">
      ...
      <div className="px-4 py-12 max-w-2xl mx-auto w-full">
        <h2 className="text-[22px] leading-[28px] font-medium text-center mb-2" style={{ color: 'var(--lj-text)' }}>
          On average, our clients save <span style={{ color: 'var(--lj-accent)' }}>{COMPARISON.savingsAmount}</span> per piece.
        </h2>
        <p className="text-[16px] leading-[24px] text-center mb-6" style={{ color: 'var(--lj-muted)' }}>
          Compared to traditional retail jewelry pricing.
        </p>
        ...
        <button
          onClick={handleCompareOpen}
          data-testid="savings-compare-trigger"
          className="flex items-center gap-1.5 mx-auto text-[14px] font-medium transition-colors duration-300 hover:opacity-80"
          style={{ color: 'var(--lj-accent)' }}
        >
          {compareOpen ? 'Hide comparison' : 'See a real comparison'}
          <ChevronDown
        ...
        <div
          ...
          style={{
            maxHeight: compareOpen ? '800px' : '0px',
            opacity: compareOpen ? 1 : 0,
            marginTop: compareOpen ? '24px' : '0px',
          }}
        >
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="p-5 rounded-[14px]" style={{ background: 'var(--lj-surface)', border: '1px solid var(--lj-border)' }}>
              <h4 className="text-[14px] font-medium mb-4 text-center" style={{ color: 'var(--lj-muted)' }}>
                Traditional Retail
              </h4>
              <div className="space-y-3">
                {[
                  ['Ring Type', ex.ringType],
                  ['Center Stone', ex.centerStone],
                  ['Color / Clarity', ex.colorClarity],
                  ['Metal', ex.metal],
                  ['Certification', ex.certification],
                ].map(([label, value]) => (
                  <div key={label} className="flex justify-between text-[14px]">
                    <span style={{ color: 'var(--lj-muted)' }}>{label}</span>
                    <span className="font-medium" style={{ color: 'var(--lj-text)' }}>{value}</span>
                  </div>
                ))}
                <div className="pt-3 mt-3" style={{ borderTop: '1px solid var(--lj-border)' }}>
                  <div className="flex justify-between items-center">
                    <span className="text-[14px]" style={{ color: 'var(--lj-muted)' }}>Price</span>
                    <span className="text-[20px] font-semibold" style={{ color: 'var(--lj-text)' }}>{ex.retailPrice}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* The Local Jewel */}
            <div className="p-5 rounded-[14px] relative" style={{ background: 'var(--lj-surface)', border: '2px solid var(--lj-accent)' }}>
              {/* Accent top bar */}
              <div className="absolute top-0 left-4 right-4 h-0.5" style={{ background: 'var(--lj-accent)' }} />
              <h4 className="text-[14px] font-medium mb-4 text-center" style={{ color: 'var(--lj-accent)' }}>
                The Local Jewel
              </h4>
              <div className="space-y-3">
                {[
                ...
                <div className="pt-3 mt-3" style={{ borderTop: `1px solid rgba(15,94,76,0.2)` }}>
                  <div className="flex justify-between items-center">
                    <span className="text-[14px]" style={{ color: 'var(--lj-muted)' }}>Price</span>
          ...
          <div className="mt-5 text-center">
            <p className="text-[16px] leading-[24px] mb-1" style={{ color: 'var(--lj-text)' }}>
              Estimated savings on this example:{' '}
              <span className="font-semibold text-[18px]" style={{ color: 'var(--lj-accent)' }}>{ex.savings}</span>
            </p>
          </div>
          ...
          <div className="mt-5 flex justify-center">
            <button
              ...
              className="min-h-[48px] px-8 rounded-[14px] font-medium text-[16px] flex items-center justify-center gap-2 transition-all duration-300 active:scale-[0.99]"
              style={{ background: 'var(--lj-accent)', color: '#FFFFFF', boxShadow: '0 4px 20px rgba(15,94,76,0.2)' }}
            >
              See my savings <ArrowRight size={18} />
            </button>
          </div>

          {/* Credibility note */}
          <p className="mt-4 text-[12px] text-center" style={{ color: 'var(--lj-muted)', opacity: 0.7 }}>
