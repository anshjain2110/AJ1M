...
agent_communication:
    ...
    -agent: "main"
    -message: "POLISH PASS complete. (1) Homepage First-Load JS reduced 391 kB -> 213 kB (~45%) by code-splitting the multi-step wizard screens (LandingScreen stays eager/SSR; other screens lazy) — verified stable by testing agent (wizard start+advance PASS, no chunk errors). (2) No hydration ERROR in production across multiple prod (next start) loads — the earlier warning was dev-mode verbose diffing only. (3) Product images are >=500px (Etsy 794px source + seed data). Added apex->www 301 middleware (inert until production apex). Everything is built + running on localhost:3001 for review. Live CRA at /app/frontend remains UNTOUCHED. NEXT STEP awaiting user approval: CUTOVER (move Next into /app/frontend, runtime swap with CRA kept for rollback, activate non-www->www 301, route /admin/* to legacy CRA, point NEXT_REVALIDATE_URL + CORS at production domain). Not started."

    -agent: "main"
    -message: "CUTOVER EXECUTED & VERIFIED (2026-06). Physical swap done: /app/frontend is now the Next.js 15 app (supervisor 'frontend' = next start on :3000); the legacy CRA was moved to /app/frontend-legacy and is served by a NEW supervisor program 'legacy-cra' (serve -s build -l 3002). next.config.js beforeFiles rewrites reverse-proxy /admin/*, /pitch(+/*), /thank-you, /privacy, /terms, /cuts, /products/*, /projects, /projects/:slug/v2, and /static/* to http://localhost:3002. Backend NEXT_REVALIDATE_URL repointed to http://localhost:3000/api/revalidate; robots adds /pitch; package.json start binds -H 0.0.0.0; created /app/frontend/.env for platform. VERIFIED on the LIVE external URL: SSR / + /blog + /collections + /projects/[slug] + /contact (www canonical, ld+json), /sitemap.xml + /robots.txt, /admin/login->/admin dashboard through the proxy (admin auth e2e), OTP /login->/dashboard, and cart->Stripe /api/checkout/session returns a real cs_test_ checkout.stripe.com URL. All legacy public pages render (no 404). Rollback documented in /app/memory/PRD.md."
    -agent: "main"
    -message: "RE: iteration_18 reported a PDP 'regression' (no metal/carat/v2-add-to-bag on /projects/4-41... and /projects/5-carat-oval-solitaire...). This was a FALSE POSITIVE: those two slugs are product_type=custom_project (non-buyable portfolio pieces) and correctly render the 'Start a piece like this' lead-gen showcase (ProjectDetailPageV2 line 302-303: if (!project.buyable)). The ACTUAL buyable shop products are /projects/2-carat-oval-hidden-halo-engagement-ring and /projects/3-carat-radiant-solitaire-engagement-ring (collections + price_matrix). NOTE: the LIST endpoint /api/projects returns buyable=None for all (not computed there); the SINGLE endpoint /api/projects/{slug} computes it correctly. Confirmed the shop PDP renders full metal(Silver/10K/14K/18K/Platinum)+carat selectors, $1,800+ From price, v2-add-to-bag, and full Product/ProductGroup/Offer schema; cart+Stripe checkout succeed on the live URL. No cutover regression. Catalog left unchanged."
